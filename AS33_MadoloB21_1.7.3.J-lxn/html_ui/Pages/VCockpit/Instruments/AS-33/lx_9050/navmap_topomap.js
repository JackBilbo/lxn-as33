class navmap {
    ...
    load_map() {
        ...
        if (this.load_map_called == null || this.load_map_called < 1) {
            this.load_map_called = this.load_map_called == null ? 1 : this.load_map_called + 1;

            document.querySelector(".loader .bar").style.width = (this.load_map_called * 1) + "%";

            if (this.load_map_called == 1) { // this is experimental code to delay the
                // Map elements
    ...
    update_map() {
        if(B21_SOARING_ENGINE.task_active() && !this.resetted) {
            this.resetMap();
            this.resetted = true;
        }
    ...
    update_map_center() {
        if(this.mapIsPanning) {
            // map is moved manually
            document.getElementById("glidericon").style.display = "none";
            document.getElementById("hawk").style.display = "none";
            TOPOMAP.setZoom(this.map_zoom);
        } else {
            document.getElementById("glidericon").style.display = "block";
        ...
        let mapoffset = (this.instrument.nav_y_offset_px - 2000) / 2;
        let glideroffset = ((1024 - this.instrument.nav_y_offset_px) / 2);

        mapoffset = this.map_rotation == "trackup" ? mapoffset + (this.instrument.nav_y_offset_px / 4) : mapoffset;
        glideroffset = this.map_rotation == "trackup" ? glideroffset - (this.instrument.nav_y_offset_px / 4) : glideroffset;
            
        document.getElementById("Map").style.top = mapoffset + "px";
        document.getElementById("glidericon").style.top = "calc(50% - " + glideroffset + "px)";
        document.getElementById("hawk").style.top = "calc(50% - " + glideroffset + "px)";
    ...
    paintMultiplayers(userid, data) {
        ...
        if(this.mpMarker[userid] == null) {
            ...
            if(this.map_rotation == "trackup") {
                label.style.transform = "rotate(" + SimVar.GetSimVarValue("A:PLANE HEADING DEGREES TRUE","degrees") + "deg)";
            } else {
    ...
    scroll_map(x,y) {
        ...
        this.mapIsPanning = true;
        TOPOMAP.panBy([x, y],{animate: true});
    }
