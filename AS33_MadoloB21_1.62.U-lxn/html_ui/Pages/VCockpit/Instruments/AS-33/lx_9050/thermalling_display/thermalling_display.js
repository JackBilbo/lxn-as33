...
class ThermallingDisplay {
    ...
    thermals_init() {
        if (this.thermals_init_completed==null) {
            ...
            this.THERMALS_GRAPH_ORIGIN_X = 400; // Set svg 0,0 point for graph
            this.THERMALS_GRAPH_ORIGIN_Y = 85;
            this.THERMALS_GRAPH_SCALE_X = 165 / this.THERMALS_GRAPH_DURATION_S; // pixels per s (5 min = 150px)
            this.THERMALS_GRAPH_SCALE_Y = -80 / 5; // pixels per ms (5 ms = 80px)
            ...
            this.THERMALS_BARS_ORIGIN_X = 35; // Set svg 0,0 point for bars
            this.THERMALS_BARS_ORIGIN_Y = 85;
            this.THERMALS_BARS_SCALE_X = 362 / this.THERMALS_BARS_DURATION_S; // pixels per s (40 min = 315px)
            this.THERMALS_BARS_SCALE_Y = -80 / 5; // pixels per ms (5 ms = 80px)

            // Blobs - these are the red/green circles drawn on the thermalling display
            this.THERMALS_BLOBS_MIN_PX = 3;  // ensure all blobs have min radius 3px
            this.THERMALS_BLOBS_MAX_PX = 30; // cap blobs to max radius 25px
            this.THERMALS_BLOBS_SCALE_PX = 6; // pixel radius per m/s climb/sink
    ...
    thermals_update_glider() {
        this.thermals_glider_svg_el.setAttribute("transform", "rotate(" + this.instrument.PLANE_HEADING_DEG.toFixed(1) + ",235,230)");
    }
    ...
    thermals_update_wind() {
        ...
        if (this.thermals_wind_units_prev == null || this.thermals_wind_units_prev != this.instrument.speed_units) {
            ...
            this.thermals_wind_units_el.innerHTML = this.instrument.speed_units == "kph" ? "KPH" : "KNOTS";
        }

    }

    thermals_update_climb() {
        let climb_value = this.thermals_smooth_te_ms * (this.instrument.climb_units == "ms" ? 1 : this.instrument.MS_TO_KNOTS);
    ...
    thermals_calc_tru_avg() {
        ...
        let avg_value = this.thermals_tru_avg_ms * (this.instrument.climb_units == "ms" ? 1 : this.instrument.MS_TO_KNOTS) ;
    ...
    thermals_update_mc() {
        ...
        if (this.thermals_mc_units_prev == null || this.thermals_mc_units_prev != this.instrument.climb_units) {
            ...
            this.thermals_mc_units_el.innerHTML = this.instrument.climb_units == "ms" ? "M/S" : "KNOTS";
        }
    }
